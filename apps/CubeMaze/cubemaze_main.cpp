...
public:
	...
	void BindShaderConstants()
	{
		...
		ball_shader_->Bind();
		ball_shader_->Uniform3f("u_light.color", 1.0f, 1.0f, 1.0f);
		ball_shader_->Uniform3f("u_light.direction", 0.825f, 0.564f, 0.0f);

		ball_shader_->Uniform1i("u_diffuse_env_sampler", 0);
		ball_shader_->Uniform1i("u_specular_env_sampler", 1);
		ball_shader_->Uniform1i("u_preintegrated_fg_sampler", 2);
		ball_shader_->Uniform4f("u_albedo_color", 0.776f, 0.776f, 0.784f, 1.0f);
		ball_shader_->Uniform1f("u_roughness", 0.112f);
		ball_shader_->Uniform1f("u_metalness", 1.0f);

		maze_shader_->Bind();
		//maze_shader_->Uniform3f("u_light.color", 1.0f, 1.0f, 1.0f);
		//maze_shader_->Uniform3f("u_light.direction", 0.825f, 0.564f, 0.0f);

		maze_shader_->Uniform1i("u_specular_env_sampler", 0);
		//maze_shader_->Uniform1i("u_diffuse_env_sampler", 0);
		//maze_shader_->Uniform1i("u_specular_env_sampler", 1);
		//maze_shader_->Uniform1i("u_preintegrated_fg_sampler", 2);
		//maze_shader_->Uniform4f("u_albedo_color", 0.219f, 0.219f, 0.219f, 1.0f);
		//maze_shader_->Uniform1f("u_roughness", 0.298f);
		//maze_shader_->Uniform1f("u_metalness", 0.0f);
	...
	bool Load() final
	{
		...
		if (!renderer_->AddShader(ball_shader_, "data/shaders/apps/CubeMaze/pbr_color")) return false;
		if (!renderer_->AddShader(maze_shader_, "data/shaders/apps/CubeMaze/glass_simple")) return false;
	...
	void RenderFloorAndWalls()
	{
		...
		maze_shader_->Uniform3fv("u_camera.position", *camera_manager_->position());
	...
	void RenderBall()
	{
		...
		ball_shader_->Uniform3fv("u_camera.position", *camera_manager_->position());
