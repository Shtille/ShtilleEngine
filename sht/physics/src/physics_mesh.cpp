...
namespace sht {
	namespace physics {
		...
		, mass_(mass)
		{
			...
			{
				const math::Vector3& mesh_vertex0 = vertices[i + 0];
				const math::Vector3& mesh_vertex1 = vertices[i + 1];
				const math::Vector3& mesh_vertex2 = vertices[i + 2];
		...
		, mass_(mass)
		{
			...
			while (enumerator->GetNextObject(&vertices_info))
			{
				...
				{
					const math::Vector3& mesh_vertex0 = vertices_info.vertices[i + 0].position;
					const math::Vector3& mesh_vertex1 = vertices_info.vertices[i + 1].position;
					const math::Vector3& mesh_vertex2 = vertices_info.vertices[i + 2].position;
					btVector3 vertex0(mesh_vertex0.x, mesh_vertex0.y, mesh_vertex0.z);
					btVector3 vertex1(mesh_vertex1.x, mesh_vertex1.y, mesh_vertex1.z);
					btVector3 vertex2(mesh_vertex2.x, mesh_vertex2.y, mesh_vertex2.z);
					triangle_mesh_->addTriangle(vertex0, vertex1, vertex2);
